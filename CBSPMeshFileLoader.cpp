...
IAnimatedMesh* CBSPMeshFileLoader::createMesh(io::IReadFile* file)
{
	...
	// read first 4 bytes to see what BSP flavor, then reset file position
	file->read( &bspID, sizeof(bspID) );
	file->seek( 0, false );

	// for IBSP or RBSP use Q3 loader
	if ( bspID == 0x50534249 || bspID == 0x50534252 )
	{
	...
	// for 2015 use MOH loader
	else if ( bspID == 0x35313032 )
	{
		CMOHLevelMesh* m = 0;

		m = new CMOHLevelMesh(FileSystem, SceneManager, LoadParam);
